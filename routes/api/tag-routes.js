...
router.get('/', (req, res) => {
  ...
  Tag.findAll({include: [Category, {model: Product, through: ProductTag}]}).then(data => res.json(data))
...
router.get('/:id', (req, res) => {
  ...
  // be sure to include its associated Product data
  // will be like product routes bc need through asso
  Tag.findOne({where:{id: req.params.id} ,include: [Category, {model: Product, through: ProductTag}]}).then(data => res.json(data))
  .catch(err => res.json(err));
});
